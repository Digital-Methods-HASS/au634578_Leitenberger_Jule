...
'If you change the code, and take out the scale_x_log_10, it gets really hard to read and understand
```{r}
theme_set(theme_bw())  # set theme to white background for better visibility
...
3. *Differentiate the **continents** by color, and fix the axis labels and units to be more legible (**Hint**: the 2.50e+08 is so called "scientific notation", which you might want to eliminate)*
  ...
  # After I eliminated the pop and the added the color, it looks more legiable
...
```{r start}
anim <- ggplot(gapminder, aes(gdpPercap, lifeExp,)) +
  geom_point() +
